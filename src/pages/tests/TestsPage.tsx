...
export function TestsPage() {
  ...
  const [editingTest, setEditingTest] = useState<ComplementarioPrueba | null>(
    null,
  );
  ...
  const handleDeleteAssignment = async (id: number) => {
    if (!confirm("¿Seguro que deseas eliminar esta asignación de prueba?"))
      return;
    try {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Gestión de evaluaciones institucionales
          </p>
        </div>
      ...
      {assignments.length > 0 && (
        ...
        >
          ...
          <div className="flex-1">
            <p
              className={`font-semibold ${moduloBloqueado ? "text-red-800" : "text-green-800"}`}
            >
              {moduloBloqueado
            ...
            <p className="text-sm text-gray-600 mt-0.5">
              {pagadas} pagadas · {pendientes} pendientes · {assignments.length}{" "}
              total
            </p>
          </div>
          <div className="flex gap-4 text-sm text-right">
            ...
            <div>
              ...
              <p
                className={`font-bold ${moduloBloqueado ? "text-red-600" : "text-green-600"}`}
              >
