...
import { PaymentModal } from "../../../features/tests/ui/PaymentModal";
...
export function TestsPage() {
  const {
    assignments, availableTests, grados, periodos, estudiantes,
    loading, pendientes, pagadas, totalRecaudo, totalPendiente, moduloBloqueado,
    assignIndividual, assignMassive, registerPayment,
    deleteAssignment, deleteComplementary, updateComplementary, createComplementary,
  } = useTests();
  ...
  const handleDeleteTest = async (id: number) => {
    if (!confirm("¿Seguro que deseas borrar este tipo de prueba? Se borrarán también las asignaciones relacionadas.")) return;
    try { await deleteComplementary(id); }
    catch { alert("Error al eliminar la prueba"); }
  };
    ...
    },
    {
      ...
      render: (_: unknown, item: PruebaAssignment) => (
        <div className="flex gap-3 items-center">
          ...
          <button onClick={() => { void handleDeleteAssignment(item.id); }}
            className="text-red-500 hover:text-red-700 transition-colors" title="Eliminar asignación">
  ...
  const filteredAssignments = assignments.filter(a => {
    ...
    const matchesQuery = !q || (a.documento?.toLowerCase().includes(q) ?? false) || (a.estudianteNombre?.toLowerCase().includes(q) ?? false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setShowCreate(true); setShowAssign(false); setEditingTest(null); }}
            className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 flex items-center gap-2 transition-colors">
            <Plus className="w-4 h-4" /> Crear Prueba
          </button>
          <button onClick={() => { setShowAssign(true); setShowCreate(false); setEditingTest(null); }}
      ...
      {assignments.length > 0 && (
        <div className={`rounded-xl p-4 flex items-center gap-4 border ${moduloBloqueado ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
          <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${moduloBloqueado ? "bg-red-100" : "bg-green-100"}`}>
            {moduloBloqueado
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 mt-0.5">{pagadas} pagadas · {pendientes} pendientes · {assignments.length} total</p>
          </div>
          <div className="flex gap-4 text-sm text-right">
            ...
            <div>
              ...
              <p className={`font-bold ${moduloBloqueado ? "text-red-600" : "text-green-600"}`}>${totalPendiente.toLocaleString()}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {(showCreate || editingTest) && (
        <NewTestForm
          ...
          onCancel={() => { setShowCreate(false); setEditingTest(null); }}
          onSave={() => { setShowCreate(false); setEditingTest(null); }}
      ...
      {payingTest && (
        <PaymentModal item={payingTest} onClose={() => { setPayingTest(null); }} onConfirm={handlePayConfirm} />
      )}

      {/* Available Tests Cards */}
      <div>
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Pruebas Disponibles</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {availableTests.map((prueba) => (
            <div key={prueba.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow relative group">
              <div className="flex items-start justify-between">
                <div>
                  <h4 className="font-semibold text-gray-900 pr-16">{prueba.nombre}</h4>
                  <p className="text-lg font-bold text-blue-600 mt-2">${prueba.valor.toLocaleString()}</p>
                  <p className="text-xs text-gray-400 mt-1">Año {prueba.anio}</p>
                </div>
                <div className="absolute top-4 right-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingTest(prueba); }}
                    className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-colors" title="Editar Prueba">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => { void handleDeleteTest(prueba.id); }}
                    className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors" title="Eliminar Prueba">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <input type="text" placeholder="Buscar por documento o nombre..." value={searchQuery}
