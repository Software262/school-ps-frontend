...
import { Home, Shield, CreditCard, Calendar, GraduationCap, Armchair, FileText, Dumbbell, Trophy, Coffee, School, Music, Building } from "lucide-react";

const modules = [
  { path: "/dashboard", label: "Dashboard", icon: Home },
  { path: "/dashboard/paz-y-salvo", label: "Paz y Salvo Central", icon: Shield, highlight: true },
  { path: "/dashboard/matricula", label: "Matrícula", icon: CreditCard },
  { path: "/dashboard/pension", label: "Pensión", icon: Calendar },
  { path: "/dashboard/escuelas-formacion", label: "Escuelas de Formación", icon: GraduationCap },
  { path: "/dashboard/salon-tesoreria", label: "Salón Tesorería", icon: Armchair },
...
export function Sidebar({ isOpen }: SidebarProps) {
  return (
    <aside className={`${isOpen ? "w-64" : "w-0"} bg-white border-r transition-all duration-300 overflow-hidden flex flex-col`} style={{ borderColor: '#d0d0ce' }}>
      <div className="p-4 border-b flex-shrink-0" style={{ borderColor: '#d0d0ce', backgroundColor: '#8E2A25' }}>
        <h1 className="font-bold text-xl text-white">SchoolPS</h1>
        <p className="text-sm" style={{ color: '#E9E9E7' }}>Sistema de Paz y Salvo</p>
      </div>
