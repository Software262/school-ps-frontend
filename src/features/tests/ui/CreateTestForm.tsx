...
interface CreateTestFormProps {
  ...
  onAssignMassive: (req: MassiveAssignRequest) => Promise<{ success: boolean; assigned: number; skipped: number; message: string }>;
}

export function CreateTestForm({
  onCancel, onSave,
  availableTests, grados, periodos, estudiantes,
  onAssignIndividual, onAssignMassive,
}: CreateTestFormProps) {
  ...
  const filteredStudents = estudiantes.filter(s => {
    ...
    const textMatch = !search || s.nombre.toLowerCase().includes(search) || s.documento.toLowerCase().includes(search);
  ...
  const handleSubmitMassive = async () => {
    ...
    if (result.assigned === 0 && result.skipped > 0) {
      setErrorMsg(`Todos los estudiantes (${result.skipped.toString()}) ya tienen esta prueba asignada.`);
    } else {
      if (result.skipped > 0) setErrorMsg(`Se asignaron ${result.assigned.toString()}. ${result.skipped.toString()} ya la tenían y fueron omitidos.`);
      onSave();
    }
  };
    ...
    <>
      <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6 shadow-sm">
        ...
        <div className="flex gap-2 mb-5">
          <button onClick={() => { setMode("massive"); setErrorMsg(""); }}
          ...
          <button onClick={() => { setMode("individual"); setErrorMsg(""); }}
        ...
        {errorMsg && (
          <div className="mb-4 p-3 bg-amber-50 border border-amber-300 rounded-lg text-sm text-amber-800">{errorMsg}</div>
        )}

        <div className={`grid grid-cols-1 gap-4 ${mode === "individual" ? "md:grid-cols-3" : "md:grid-cols-3"}`}>
          {/* Col 1 */}
          {mode === "massive" ? (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Seleccionar Grado Objetivo</label>
            ...
            <>
              <div>
                ...
                <select value={indGradoId} onChange={e => { setIndGradoId(e.target.value); setStudentId(""); }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                  ...
                  {filteredStudents.length === 0 ? (
                    ...
                    filteredStudents.map(s => {
                      ...
                      return (
                        ...
                        >
                          <span>
                            <span className="font-mono text-xs text-gray-400 mr-2">{s.documento}</span>
                            {s.nombre}
                          </span>
                          {isSelected && (
                            <svg className="w-4 h-4 text-blue-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
          ...
          {/* Prueba */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Seleccionar Prueba</label>
          ...
          {/* Período */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Periodo Académico (Opcional)</label>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            ...
            {loading ? "Asignando..." : mode === "massive" ? "Asignar a todo el grado" : "Asignar al estudiante"}
          </button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-5 space-y-1.5 text-sm">
              <p className="text-gray-700 mb-2">Se asignará a todos los activos que <strong>aún no la tengan</strong>.</p>
              {[
            ...
            <div className="flex justify-end gap-3">
              ...
              <button onClick={() => { void handleSubmitMassive(); }}
