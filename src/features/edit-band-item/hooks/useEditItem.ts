...
import { editItem } from '../api/edit-item';

export const useEditItem = (item: Inventory | null, onSuccess: () => void) => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      setErrors({ general: 'No se pudo actualizar el instrumento. Intenta de nuevo.' });
    } finally {
      setLoading(false);
    }
  };
